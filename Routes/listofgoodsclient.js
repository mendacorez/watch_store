...
router.post("/", async(req, res) => {
    try {
        const {watch_name, manufacturer_id, mechanism_type_id, strap_type_id, color_id, gender_id, watch_price, discount_size, price_with_discount, watch_quantity} = req.body
        const addGood = await pool.query(
            ...
            [watch_name, manufacturer_id, mechanism_type_id, strap_type_id, color_id, gender_id, watch_price, discount_size, price_with_discount, watch_quantity])
...
router.put("/:watch_id", async(req, res) => {
    try {
        ...
        const updateGood = await pool.query(
            ...
            [watch_id, watch_name, manufacturer_id, mechanism_type_id, strap_type_id, color_id, gender_id, watch_price, discount_size, price_with_discount, watch_quantity]
        )
...
router.get('/sortBy/alphabet', async (req, res) => {
    try{
        const {watch_name} = req.params
        const watch = await pool.query("SELECT * FROM listofgoods ORDER BY $1", [watch_name]);
...
router.get('/sortBy/priceAsc', async (req, res) => {
    try{
        const {price_with_discount} = req.params
        const price = await pool.query("SELECT * FROM listofgoods ORDER BY $1", [price_with_discount]);
...
router.get('/sortBy/priceDesc', async (req, res) => {
    try{
        const { price_with_discount } = req.params
        const price = await pool.query("SELECT * FROM listofgoods ORDER BY $1 DESC", [price_with_discount]);
        await res.json(price.rows);
    }
    catch (e) {
        console.log(e.message);
